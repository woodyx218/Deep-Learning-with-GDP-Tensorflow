...
def main(unused_argv):
    ...
    for epoch in range(1, FLAGS.epochs + 1):
        ...
        for step in range(steps_per_epoch):
            tf.set_random_seed(0)
            whether=np.random.random_sample(60000)>(1-256/60000)
            subsampling=[i for i in np.arange(60000) if whether[i]]
            global microbatches
        ...
        if FLAGS.dpsgd:
            fffff=open(os.getcwd() + '/PoissonMNIST_'+str(FLAGS.noise_multiplier)+ '.pkl', 'wb')
        else:
            fffff=open(os.getcwd() + '/PoissonMNIST_nonprivate.pkl', 'wb')
